...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            # 一个回合的长度
            episode_len = original_action_shape[0]  # max_episode
            
            if sample_full_episode:
                start_ts = 0
            else:
                start_ts = np.random.choice(episode_len) # 随机抽取一个索引
        ...
        # print(os.path.join(os.path.dirname(os.path.abspath(__file__)), "utils.py"))
        # exit(-1)
        ...
        # print("original_action_shape: ", original_action_shape)
        # torch.Size([3, 3, 480, 640]) torch.Size([14]) torch.Size([100, 14]) torch.Size([100])
