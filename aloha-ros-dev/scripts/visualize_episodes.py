...
STATE_NAMES = JOINT_NAMES + ["joint6"]
...
def main(args):
    ...
    dataset_name = os.path.join(task_name, dataset_name)
...
def save_videos(video, dt, video_path=None):
    if isinstance(video, list):
        ...
        all_cam_videos[0], all_cam_videos[1] = all_cam_videos[1], all_cam_videos[0] # 交换top和left图像位置
        ...
        for t in range(n_frames):
            image = all_cam_videos[t] # opencv原本就是bgr顺序
            image = image[:, :, [2, 1, 0]]  # swap B and R channel  rgb
            # image = image[:, :, [0, 1, 2]] # bgr
            cv2.imshow("imges", image)
            if(cv2.waitKey(30) == 'q'):
                exit(-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', action='store', type=str, default="./data", help='Dataset dir.', required=False)
    parser.add_argument('--task_name', action='store', type=str, help='Task name.', default="aloha_mobile_dummy", required=False)
    parser.add_argument('--episode_idx', action='store', type=int,  default=0, help='Episode index.', required=False)
