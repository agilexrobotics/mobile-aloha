...
arx_arm::arx_arm(int CONTROL_MODE)
{
    ...
    if (modifyLinkMass(model_path, model_path, 0.581)) { //默认单位kg 夹爪质量 0.381 0.581
...
void arx_arm::update_real(command cmd)
{
    ...
    solve.fk_calc(arx5_cmd,current_pos,0,is_teach_mode);
    ...
    else
    {
        if(control_mode == 0  ||  control_mode == 1)//解算
        {
                ...
                if(teach2pos_returning ){
                    ...
                    is_starting=1;
                    arx5_cmd.reset = true;
                    arx5_cmd.waist_pitch  = arx5_cmd.waist_pitch_t   = joy_pitch_t  =joy_pitch      =0      ;
                    arx5_cmd.x            = arx5_cmd.x_t             = joy_x_t      =joy_x          =0  ;
                    arx5_cmd.y            = arx5_cmd.y_t             = joy_y_t      =joy_y          =0  ;
                    arx5_cmd.z            = arx5_cmd.z_t             = joy_z_t      =joy_z          =0  ;      
                    arx5_cmd.base_yaw     = arx5_cmd.base_yaw_t      =      0 ;
                    arx5_cmd.gripper      = arx5_cmd.gripper_t       =      0 ;
                    arx5_cmd.gripper_roll = arx5_cmd.gripper_roll_t  = joy_roll_t   =joy_roll       =0      ;
                    arx5_cmd.waist_yaw    = arx5_cmd.waist_yaw_t     = joy_yaw_t    =joy_yaw        =0      ;
                    arx5_cmd.mode = FORWARD;
                }
                else{
                    is_starting=0;
...
void arx_arm::motor_control()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        if(control_mode == 0 || control_mode ==2){  //arx5
            if(i>2){
                slove_cur_t[i] = solve.jointTorques(i) * 2.36; //2.45  2.3 
            }else{slove_cur_t[i] = solve.jointTorques(i) * 0.72f; }            
        ...
        if (slove_cur_t[i] >= max_torque)
            slove_cur_t[i] = max_torque;
        else if (slove_cur_t[i] <= -max_torque)
            slove_cur_t[i] = -max_torque;
        
        slove_cur_t[i] = slove_cur_t[i] * (1.0 - filter_torque) + solve.jointPrevTorques(i) * filter_torque;
        solve.jointPrevTorques(i) = slove_cur_t[i];
    }

    for(int i=0;i<6;i++){
        slove_cur[i]=slove_cur_t[i];
    }
    ...
    if(current_normal)
    {
            ...
            ////////////////////////////force/////////////////////////////////////
            if(control_mode == 0 || control_mode==2){
                CAN_Handlej.Can_cmd_all (1, 150, 12, target_pos_temp[0], 0, solve.jointTorques(0));
            }else if(control_mode == 1 || control_mode==3){
                CAN_Handlej.Send_DM_Moto(1, 39, 0.8, target_pos_temp[0], 0, solve.jointTorques(0));
            }
            usleep(200);
            CAN_Handlej.Can_cmd_all (2, 150, 12, target_pos_temp[1], 0, solve.jointTorques(1));
            usleep(200);
            CAN_Handlej.Can_cmd_all (4, 150, 12, target_pos_temp[2], 0, solve.jointTorques(2));
            usleep(200);
            CAN_Handlej.Send_DM_Moto(5, 20, 0.8, target_pos_temp[3], 0, solve.jointTorques(3)); //20 0.8   25  0.8
            usleep(200);
            CAN_Handlej.Send_DM_Moto(6, 20, 0.8, target_pos_temp[4], 0, solve.jointTorques(4));
            usleep(200);
            CAN_Handlej.Send_DM_Moto(7, 20, 1  , target_pos_temp[5], 0, solve.jointTorques(5));
            usleep(200); 
            // if(fabs(current_torque[6])>2)
            // {
            //     follow_pos_k-=0.1f;
            // }
            // else follow_pos_k+=0.05f;
            // if(follow_pos_k<0.1)
            // follow_pos_k=0.1;

            // CAN_Handlej.Send_DM_Moto(8, follow_pos_k, 0.1 , target_pos_temp[6], 0, 0);
            // usleep(200); 


            // if(fabs(current_torque[6])>=2)
            // {
            //     CAN_Handlej.Send_DM_Moto(8, 0, 0 , 0, 0, 2);
            // }
            // else {
            //     CAN_Handlej.Send_DM_Moto(8, 1, 0.1 , target_pos_temp[6], 0, 0);
            // }
            float temp_k=8.0f;  //2.0f  3
            float send_cur=0;
            send_cur=temp_k*(target_pos_temp[6]-0.5-current_pos[6]);
            send_cur=limit<float>(send_cur,-4,4);

            CAN_Handlej.Send_DM_Moto(8, 0, 0.1, 0, 0,send_cur );  //0.1

            // CAN_Handlej.Send_DM_Moto(8, 0, 0.1, 0, 0,send_cur );  //0.1
  

            ROS_INFO("gripper pos>>%f cur>>%f  send>>%f",current_pos[6],current_torque[6],send_cur);
    ...
    {               
                    ...
                    usleep(200); 
                    CAN_Handlej.Send_DM_Moto(8, 0, 1, 0, 0, 0);
                    usleep(200); 
...
void arx_arm::joint_control()
{
        ...
        float ramp_ros_pos_k2=1.0f;//0.005f  0.01
...
{
            if(Teleop_Use()->axes_[2]<0  || arx5_cmd.key_o  ||  ros_control_pos_t[6] >=250000) //开启
            gripper_spd=45;
            else if (Teleop_Use()->buttons_[4] == 1 || arx5_cmd.key_c  ||   ros_control_pos_t[6] <250000) //夹取
            gripper_spd=-45;
            ...
            ROS_INFO("gripper %f",ros_control_pos_t[6]);
...
void arx_arm::arm_teach_mode(){
            ...
            // if (button_teach && !arx5_cmd.key_t && !record_mode)
