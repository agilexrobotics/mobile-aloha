...
class RealEnv:
    ...
    # 观察者空间                 
    Observation space: {"qpos": Concat[ left_arm_qpos (6),          # absolute joint position  绝对位置 
                                        left_gripper_position (1),  # normalized gripper position (0: close, 1: open)
                                        
                                        right_arm_qpos (6),         # absolute joint position
                                        right_gripper_qpos (1)]     # normalized gripper position (0: close, 1: open)
                        
                        "qvel": Concat[ left_arm_qvel (6),         # absolute joint velocity (rad)  绝对关节角速度
    ...
    def __init__(self, init_node, setup_robots=True, setup_base=False):
        ...
        # self.setup_t265()
        self.setup_dxl()  # DynamixelClient 电机通讯

        # 订阅的3个消息 从左臂爪 从右臂爪 图像
        # Subscriber JointState类型  JointGroupCommand JointSingleCommand 3个消息
        # interbotix_xs_msgs::msg::JointGroupCommand 
        # 图像：高、低left、right手腕  4个消息
    ...
    def get_base_vel(self):
        ...
        # 右边取反
        right_vel = -right_vel # right wheel is inverted  wheel_r轮子的半径 = 0.101 / 2 
        
        # 线速度
        base_linear_vel = (left_vel + right_vel) / 2 * self.wheel_r
        # 角速速度
